...
class Core(Search):
    ...
    def _get_core_async_responses(self, query: str, total_pages: int, records_per_query: int = 10) -> \
        ...
        for page in range(1, total_pages + 1):
            ...
            async_tasks.append(celery_async_requests.get.delay())

        # while True:
        #     logger.info(get_task_state_by_id(async_tasks[1].id))
        #     time.sleep(1)

        for async_task in async_tasks:
            logger.info(async_task.id)
            logger.info(async_task.wait(interval=0.5))
    ...
    def _get_core_hits(self, api_response: requests.models.Response) -> List[GeneralizedHit]:
        return [self._parse_one_core_hit(core_hit) for core_hit in api_response.json()['results']]
